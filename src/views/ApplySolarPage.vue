...
async function submitForm() {
  ...
  try {
    ...
    const { error } = await supabase.from('solar_applications').insert([{
      business_name: isBusiness ? form.value.businessName || 'Business' : 'Homeowner / Individual',
      reg_no: isBusiness ? form.value.regNo || 'N/A' : 'N/A',
      years_operating: isBusiness ? form.value.yearsOperating || 0 : 0,
      address: form.value.address || 'Not provided',
      city: form.value.city || 'Not provided',
      province: form.value.province || 'Not provided',
      full_name: form.value.fullName || 'Not provided',
      position: form.value.position || 'Not provided',
